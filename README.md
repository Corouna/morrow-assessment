...
Using the running app also surfaced two real UI problems. A long habit list pushed the add-habit form below the fold, forcing a scroll just to add a habit — I asked for it to be restructured so the form stayed reachable regardless of list length. Then, resizing the browser window down to mobile width myself, I noticed the form and the habit cards jumping to visibly different, mismatched sizes right at the responsive breakpoint — an "uncomfortable UI juggling" I flagged and asked to be fixed so the two stayed consistent at every width.
...
Three rounds of correction happened after that initial build, each driven by review and testing rather than caught in the first pass. A self-review step (the "final consistency pass" commit) caught and fixed two real bugs: an unhandled promise rejection in `AddHabitForm` — a failed post-create refetch would have thrown silently instead of showing an error — and unnecessary `as` type assertions in habit-creation validation, replaced with a type-safe discriminated union. Next, testing the running app by hand surfaced a real product gap: nothing stopped creating two habits with the same name. That was fixed with a database constraint and a `409` response, which also removed an index that actually was redundant, flagged during a SOLID/YAGNI review.

A third round was a dedicated security and duplication audit. SQL injection and XSS payloads were sent directly at the running API: injection attempts were stored as inert text (parameterized queries, never string interpolation) and confirmed the schema was untouched afterward; the XSS payload was confirmed to render as escaped literal text via a real rendered-DOM check, not assumed from "React escapes by default." CORS was verified from an actual cross-origin browser context — a same-origin `fetch` succeeds, a different-origin one is blocked with `Failed to fetch` — rather than inferred from response headers via `curl`, which doesn't enforce CORS at all and would have given a false pass. Two real gaps were found and fixed: the centralized error handler returned a generic 500 for a malformed or oversized request body instead of the 400/413 `express.json()` already signals (see [Consistency & Failure Semantics](#consistency--failure-semantics)), and the default `X-Powered-By: Express` header — a trivial stack-fingerprinting leak — was disabled. One earlier claim from the SOLID/YAGNI review was re-examined and found wrong, not just re-confirmed: `idx_habit_logs_habit_id` was called "likely redundant" at the time, but MySQL refuses to drop it (`Cannot drop index ... needed in a foreign key constraint`) — it's required by the `ON DELETE CASCADE` foreign key, since `habit_id` isn't the leftmost column of any other index on that table. Left in place, correctly this time.

A UI iteration followed, in two parts. First, a direct request to fix a real layout problem: the add-habit form sat below the habit grid, so a long list pushed it out of view. Restructured into a header/sidebar/main layout with the form sticky-positioned in the sidebar — verified by seeding 14 habits to force genuine scrolling and confirming via Puppeteer that the form stayed in the viewport both before scrolling and after scrolling all the way to the bottom, not just "looks sticky" in a screenshot. Second, testing that new layout by hand at different window widths surfaced another real gap: the sidebar's hardcoded 280px and the habit-grid's separately hardcoded 220px column minimum never matched, and the mobile breakpoint let the form stretch to the full container width with nothing capping it — producing a visible size jump right at the breakpoint. Fixed by tying the sidebar width, the grid's column minimum, and the form's own max-width to one shared CSS variable, verified by measuring actual rendered pixel widths (not just reading the CSS) across six viewports from 375px to 1100px: form and card are now exactly the same width at every mobile size, with no jump at the breakpoint.

Each of these rounds is its own commit with its own live verification — see the commit history rather than taking this summary's word for it.
